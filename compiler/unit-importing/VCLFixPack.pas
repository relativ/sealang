...
{$IF CompilerVersion > 24.00}{$IF CompilerVersion < 26.00}
  {$O-} // code optimization is not always correct
{$IFEND}{$IFEND}
...
{$IF CompilerVersion < 20.0} // Delphi 6-2007
  {+}//{$DEFINE ContextMenuFix}{+.}// not work with EntityCombo
{$IFEND}
...
{**************************************************************************************************}
{ Workaround for Windows Vista CompareString bug.                                                  }
{ The Ь/ь ($DC/$FC) and the UE/ue are treated equal in all locales, but they aren't equal. There   }
...
begin
  ...
  begin
    ...
    { Copy code points and replace "Ь" ($DC) and "ь" ($FC) with "U"/"u" + combining diaresis }
    Source := P + ValidCount;
